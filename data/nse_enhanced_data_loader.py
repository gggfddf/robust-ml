#!/usr/bin/env python3
"""
ULTIMATE MARKET AI ENGINE - NSE ENHANCED DATA LOADER
NSE-Specific Data Loading with Multi-timeframe Support
Production-Grade Implementation for Indian Markets
"""
...
try:
    ...
    print("⚠️ NSEpy not available, falling back to yfinance")
...
class NSEEnhancedDataLoader:
    """Enhanced NSE Data Loader with Multi-timeframe Support"""
    
    def __init__(self, symbol="RELIANCE"):
        self.symbol = symbol
        self.nse_symbol = f"{symbol}.NS"  # For yfinance fallback
        self.cache_dir = "cache"
        self.data_cache = {}
        
        # Timeframe configurations for NSE
        self.timeframes = {
            "1d": {"period": "10y", "interval": "1d", "max_records": 2500},
            "1w": {"period": "10y", "interval": "1wk", "max_records": 520},
            "1m": {"period": "1y", "interval": "1m", "max_records": 52560},
            "5m": {"period": "1y", "interval": "5m", "max_records": 52560},
            "15m": {"period": "1y", "interval": "15m", "max_records": 52560},
            "30m": {"period": "1y", "interval": "30m", "max_records": 52560},
            "1h": {"period": "1y", "interval": "1h", "max_records": 52560}
        }
        
        # Ensure cache directory exists
        os.makedirs(self.cache_dir, exist_ok=True)
        
        logger.info(f"NSE Enhanced DataLoader initialized for symbol: {symbol}")
        logger.info(f"NSEpy available: {NSEPY_AVAILABLE}")
    
    def load_multi_timeframe_data(self) -> Dict[str, pd.DataFrame]:
        """Load data for all timeframes"""
        logger.info("Loading multi-timeframe NSE data...")
    ...
    def load_data(self, timeframe: str, symbol: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            data = self._fetch_nse_data(symbol, timeframe)
    ...
    def _fetch_nse_data(self, symbol: str, timeframe: str) -> Optional[pd.DataFrame]:
        """Fetch data from NSE using appropriate method"""
        try:
            ...
            # Use NSEpy for daily data, yfinance for intraday
            if timeframe in ["1d", "1w"] and NSEPY_AVAILABLE:
                data = self._fetch_nsepy_data(symbol, start_date, end_date, timeframe)
            else:
    ...
    def _fetch_nsepy_data(self, symbol: str, start_date: datetime, end_date: datetime, timeframe: str) -> Optional[pd.DataFrame]:
        """Fetch data using NSEpy"""
        try:
            # Convert to date objects for NSEpy
            ...
            logger.error(f"❌ Error fetching NSEpy data: {str(e)}")
            return None
    
    def _fetch_yfinance_data(self, symbol: str, start_date: datetime, end_date: datetime, timeframe: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            ticker = yf.Ticker(self.nse_symbol)
            data = ticker.history(
                start=start_date,
                end=end_date,
                interval=interval,
    ...
    def _get_cache_file(self, symbol: str, timeframe: str) -> str:
        ...
        return os.path.join(self.cache_dir, f"NSE_{symbol}_{timeframe}_{end_date}.pkl")
...
def main():
    """Test the NSE enhanced data loader"""
    print("🚀 Testing NSE Enhanced Data Loader")
    print("=" * 50)
    ...
    # Print results
    print(f"\n📊 NSE Data Summary for {summary['symbol']}:")
